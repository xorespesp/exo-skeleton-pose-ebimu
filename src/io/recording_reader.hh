...
namespace io
{
    ...
    public:
        ...
        std::span<const recorded_camera_stream> camera_streams() const noexcept { return _streams; }

        std::chrono::nanoseconds first_timestamp() const noexcept { return _first_timestamp; }
        std::chrono::nanoseconds last_timestamp() const noexcept { return _last_timestamp; }
        ...
        void seek_timestamp(stream_id_t stream_id, std::chrono::nanoseconds timestamp) noexcept;

        // nullopt at the end of the stream. Only this stream's frames are decoded.
        [[nodiscard]] std::optional<frame> fetch_next_frame(stream_id_t stream_id) noexcept;
    ...
    private:
        ...
        void _restart_cursor(stream_id_t stream_id, std::chrono::nanoseconds from);
    ...
    private:
        ...
        std::vector<recorded_camera_stream> _streams;
        std::vector<std::unique_ptr<playback_cursor>> _cursors; // parallel to _streams

        std::chrono::nanoseconds _first_timestamp{ 0 };
        std::chrono::nanoseconds _last_timestamp{ 0 };
        bool _opened{ false };
    };
