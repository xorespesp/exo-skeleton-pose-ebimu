...
| 스트림 점프 통보 | 프레임에 세대 번호를 실어 소비자가 대조 | **프레임 사이에 끼우는 리셋 마커** | 마커는 데이터와 같은 스레드·같은 순서로 흐르므로 순서가 곧 보증이다. 대조할 신원이 필요 없어 `sensor_frame` 도 `on_sensor_frame_update` 도 안 건드리고, `on_sensor_stream_reset()` 이 이미 그 자리에 있었다 |
